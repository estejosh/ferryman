...
//! process ΓÇö it is read from env/keyring by the caller and handed to `TelegramGate::
...
//! this module logs a fixed message only ΓÇö never the error value itself.
//!
...
impl TelegramGate {
    /// Only returns `Some` when both a bot token and an approver id are configured ΓÇö
...
async fn notify_loop(client: TelegramClient, store: SqliteStore, interval: Duration) {
    ...
    loop {
        ...
        for job in pending {
            ...
            let keyboard = json!({
                "inline_keyboard": [[
                    {"text": "Γ£à Approve", "callback_data": format!("approve:{}", job.id)},
                    {"text": "≡ƒÜ½ Deny", "callback_data": format!("deny:{}", job.id)}
                ]]
            });
        ...
        for update in updates {
            ...
            let reply = match (action, outcome) {
                (GateAction::Approve, Ok(Some(job))) => {
                    format!("approved ΓÇö job {} is now queued", job.id)
                }
                (GateAction::Deny, Ok(Some(job))) => {
                    format!("denied ΓÇö job {} will not run", job.id)
                }
...
/// the outcome and drop the keyboard ΓÇö the same fail-closed authorization and store
...
async fn handle_callback_query(
    ...
    let (answer_text, edit_text) = match (action, &outcome) {
        (GateAction::Approve, Ok(Some(job))) => (
            "Approved".to_string(),
            Some(format!("Approved Γ£ô ΓÇö job {} is now queued", job.id)),
        ),
        (GateAction::Deny, Ok(Some(job))) => (
            "Denied".to_string(),
            Some(format!("Denied Γ£ù ΓÇö job {} will not run", job.id)),
        ),
...
mod tests {
    ...
    use ferryman_core::{JobStatus, NewJob, PolicyEnvelope};
    ...
    fn gated_job(store: &SqliteStore, description: &str) -> ferryman_core::Job {
        store
            .submit_job(
                "p",
                NewJob {
    ...
    /// the configured approver must be dropped ΓÇö the job must not move.
    #[tokio::test]
    ...
    /// configured approver must be dropped ΓÇö the job must not move and the message must
    /// not be edited.
    #[tokio::test]
