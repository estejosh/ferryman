...
async fn agent_command(command: Agent) -> Result<()> {
    ...
    match command {
        Agent::Run {
            ...
            //
            // Filing happens inside `route_for`, and `fleet_under` calls that once per
            // channel at startup and never again - it reuses the routes it built. A root
            // created after that has already missed the only nineteen chances it was
            // going to get, which is exactly what happened the first time this was
            // written a dozen lines lower down.
            //
            // A fleet enabled months ago has no root, so without this nothing is ever
            // filed on precisely the machines watching the most channels. Best-effort,
            // and it creates a directory and nothing else: ADR 0019 is that nothing of
            // the operator's ever moves into it.
            let made_root = if ferryman_channel::ferry::find_root().is_none() {
                comms
                    .parent()
                    .and_then(|beside| {
                        ferryman_channel::ferry::Root::create(&beside.join("ferry")).ok()
                    })
                    .map(|root| root.path)
            } else {
                None
