//! Release approval: a signed request in the channel, and a signed decision
//! that either authorises or refuses it.
//!
...
//! changelog — and a human operator approves or denies it. The primitive that makes
//! this a real gate rather than a rubber stamp is the same one that already seals an
//! operator's identity: the operator's signing key is sealed at rest with their
//! password (`ferryman-server::operators`), so the fleet cannot sign an approval
//! because it never has the password. Signing in IS the gate.
//!
...
//! - `release/decision.<id>.<operator>.json` — written once by each operator.
//!
//! A decision is a claim that an operator saw a specific release and approved (or
...
/// A signed operator decision on a request. One writer per path: the operator.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct ReleaseDecision {
    /// Which request this is about.
    pub request_id: String,
    /// The version as the operator saw it. Must match the request exactly.
    pub version: String,
    /// The commit as the operator saw it. Pinned: a decision over a different commit
    /// authorises nothing, even when its signature is perfectly valid.
    pub commit: String,
    /// `approve` or `deny`. Only `approve` authorises a release.
    pub decision: String,
    /// The operator's own words, when they want to leave any. Optional.
    #[serde(default)]
    pub reason: String,
    /// Which operator decided. Its only writer.
    pub operator: String,
...
impl ReleaseDecision {
    ...
    #[must_use]
    pub fn new(request: &ReleaseRequest, decision: &str, reason: &str, operator: &str) -> Self {
        Self {
            ...
            operator: crate::canonical_agent_name(operator),
            decided_at: Utc::now(),
            signed_by: None,
            signature: None,
        }
...
/// Where one operator's decision on one request lives. Named by both, so two operators
/// (or one operator and one impostor) never contend for the same file.
#[must_use]
pub fn decision_path(route: &ProjectRoute, id: &str, operator: &str) -> PathBuf {
    release_dir(route).join(format!(
        "decision.{id}.{}.json",
        crate::canonical_agent_name(operator)
    ))
}
...
/// purpose: that is what makes an approval a pin.
...
/// Sign and write an operator's decision. One file per operator per request.
pub fn decide_release(
    ...
    decision.operator = crate::canonical_agent_name(&decision.operator);
    ...
    let path = decision_path(route, &decision.request_id, &decision.operator);
    crate::atomic_json(&path, &decision)
        .with_context(|| format!("write release decision {}", path.display()))?;
    Ok(path)
}

/// Read one operator's decision on one request, if it exists.
pub fn read_decision(
    route: &ProjectRoute,
    id: &str,
    operator: &str,
) -> Result<Option<ReleaseDecision>> {
    let path = decision_path(route, id, operator);
    if !path.exists() {
        return Ok(None);
    }
    let text = std::fs::read_to_string(&path)
...
/// Whether a decision is what its operator signed.
#[must_use]
...
/// The newest decision for a request, if any operator has decided. Used by the caller
/// (the dashboard and `ferry release land`) to find the decision to check.
...
pub enum ReleaseRefusal {
    ...
    InvalidSignature,
    /// The decision names an operator the roster does not know.
    UnknownOperator(String),
    /// The decision was signed by a different key than the roster holds for that name.
    KeyChanged,
    /// The operator approved a different version than the request proposes.
    VersionMismatch { requested: String, approved: String },
    /// The operator approved a different commit than the request proposes. This is the
...
/// from a name the roster knows, over exactly this request's version and commit, within
/// the horizon, with green CI. Anything else is a [`ReleaseRefusal`].
pub fn authorize_release(
    ...
    // operator signed. Checking an unauthenticated field is how a forged decision gets
    ...
    match verify_decision(decision, roster) {
        ...
        SignatureCheck::UnknownSigner => {
            return Err(ReleaseRefusal::UnknownOperator(decision.operator.clone()));
        }
...
mod tests {
    ...
    fn a_request_and_an_approval_survive_the_round_trip_and_authorise() {
        ...
        let (operator, operator_entry) = signer("estejosh");
        ...
        let decision = ReleaseDecision::new(&read_back, "approve", "ship it", "estejosh");
        decide_release(&route, &decision, &operator).unwrap();
        let read_back = read_decision(&route, &request.id, "estejosh")
            .unwrap()
            .unwrap();
        assert_eq!(
            verify_decision(&read_back, std::slice::from_ref(&operator_entry)),
            SignatureCheck::Valid
        );
        ...
        assert_eq!(
            authorize_release(
                ...
                &read_back,
                std::slice::from_ref(&operator_entry),
                read_back.decided_at,
            ),
            Ok(())
        );
    }
    ...
    fn a_denial_does_not_authorise() {
        ...
        let request = green_request("grouchly");
        propose_release(&route, &request, &machine).unwrap();
        let decision = ReleaseDecision::new(&request, "deny", "not ready", "estejosh");
        decide_release(&route, &decision, &operator).unwrap();
        let decision = read_decision(&route, &request.id, "estejosh")
            .unwrap()
            .unwrap();
    ...
    fn an_approval_from_a_name_the_roster_does_not_know_is_refused() {
        ...
        let route = route(dir.path());
        let (machine, _) = signer("grouchly");
        let (operator, _) = signer("estejosh");

        let request = green_request("grouchly");
        propose_release(&route, &request, &machine).unwrap();
        let decision = ReleaseDecision::new(&request, "approve", "", "estejosh");
        decide_release(&route, &decision, &operator).unwrap();
        let decision = read_decision(&route, &request.id, "estejosh")
            .unwrap()
            .unwrap();
        ...
        assert_eq!(
            ...
            Err(ReleaseRefusal::UnknownOperator("estejosh".to_string()))
        );
    }
    ...
    fn an_approval_with_a_bad_signature_is_refused() {
        ...
        let decision = ReleaseDecision::new(&request, "approve", "", "estejosh");
        decide_release(&route, &decision, &operator).unwrap();
        let mut decision = read_decision(&route, &request.id, "estejosh")
            .unwrap()
            .unwrap();
        // The roster knows an "estejosh" with the *machine's* key, so the operator's
        // signature is invalid — a wrong key claiming a known name.
        decision.signature = Some(hex::encode([0u8; 64]));
        let entry = AgentRoute {
            name: "estejosh".to_string(),
            role: "operator".to_string(),
            capabilities: Vec::new(),
            public_key: Some(machine_entry.public_key.clone().unwrap()),
            encryption_key: None,
        };
        assert_eq!(
            authorize_release(&request, &decision, &[entry], request.created_at),
    ...
    fn an_approval_for_a_different_commit_does_not_authorise_the_request() {
        ...
        let mut moved = ReleaseDecision::new(&request, "approve", "", "estejosh");
        moved.commit = "ffffffffffffffffffffffffffffffffffffffff".to_string();
        decide_release(&route, &moved, &operator).unwrap();
        let moved = read_decision(&route, &request.id, "estejosh")
            .unwrap()
            .unwrap();
    ...
    fn editing_a_decision_after_signing_no_longer_verifies() {
        ...
        assert_eq!(
            verify_decision(&tampered, std::slice::from_ref(&operator_entry)),
            SignatureCheck::Invalid
        );
    }

    #[test]
    fn one_writer_per_path_so_two_operators_cannot_collide() {
        let dir = tempfile::tempdir().unwrap();
        let route = route(dir.path());
        let (machine, _) = signer("grouchly");
        let (first, _) = signer("estejosh");
        let (second, _) = signer("operator");

        let request = green_request("grouchly");
        propose_release(&route, &request, &machine).unwrap();
        let a = decide_release(
            &route,
            &ReleaseDecision::new(&request, "approve", "", "estejosh"),
            &first,
        )
        .unwrap();
        let b = decide_release(
            &route,
            &ReleaseDecision::new(&request, "approve", "", "operator"),
            &second,
        )
        .unwrap();
