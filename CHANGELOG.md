...
- **The dashboard operator is the seed, and the password is the local unlock.**
  ...
  "ferryman/v1/operator")`, so the recovery phrase genuinely restores the person,
  not just the agents. The password is demoted to the local unlock - it still
  ...
  the secret available over HTTP.
