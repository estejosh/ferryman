...
2. **Project compatibility records:** each private project independently declares `[updates] opt_in = true` in `bridge-project.toml`. A system-wide apply records the selected Bridge release in `.orchestrator/bridge-update-state.json`; it never changes project files, makes Git commits, adds remotes, or publishes anything.

```powershell
# Inventory eligible projects. This is read-only.
cargo run -p orchestrator-cli --bin orchestrator-updater -- check --projects-root ./.data/projects

# Explicitly record an approved release for opted-in projects only.
cargo run -p orchestrator-cli --bin orchestrator-updater -- --projects-root ./.data/projects apply --release 0.1.0

# Update the Bridge installation itself only when its checkout is clean and origin is already set.
cargo run -p orchestrator-cli --bin orchestrator-updater -- update-bridge --checkout X:\orchestrator-bridge
```
