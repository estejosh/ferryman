...
Terminate TLS at a reverse proxy and bind Ferryman only on a trusted private network. Set these environment variables before starting with `--production`:
...
Ferryman gates orchestration - leases, approvals, memory, recovery - but it does
...
- Run each worker under a dedicated, least-privilege OS account - never Ferryman's
...
- Prefer the agent's own sandbox/approval flags (set through `AGENT_ARGS_JSON`) over
  trusting Ferryman to contain the model. `AGENT_ARGS_JSON` is forwarded verbatim;
...
- use explicit database, artifact, project-workspace, memory, and recovery roots with access limited to Ferryman service identity;
